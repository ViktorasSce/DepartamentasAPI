...
{
    public class GetCourseDepartmentRepository : IGetCourseDepartmentRepository
    {
        private readonly IDbConnection _connection;

        public GetCourseDepartmentRepository(IConfiguration configuration, IDbConnection connection)
        {
        ...
        public IEnumerable<GetCourseDepartment> GetCourseByDepartmentId(int departmentid)
        {
            try
            {
                //using (var connection = new NpgsqlConnection(_configuration.GetConnectionString("PostgreConnection")))
                {
                    string sql = "SELECT public.courses.courname, public.depcourstud.departmentid, public.depcourstud.courseid FROM public.courses JOIN public.depcourstud ON public.courses.id = public.depcourstud.courseid WHERE departmentid = @departmentid";

                    var queryArguments = new
                    {
                        departmentid = departmentid
                    };
                    var result = _connection.Query<GetCourseDepartment>(sql, queryArguments);

                    if (result != null && result.Any())
                    {
                        return _connection.Query<GetCourseDepartment>(sql, queryArguments);
